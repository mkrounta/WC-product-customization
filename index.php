<?php
/**
 * Plugin Name:       Product Customization
...
*/
define( 'PC_PATH', plugin_dir_url( __FILE__ ) );
add_action( 'wp_enqueue_scripts', 'enqueue_wp_child_theme' );
function enqueue_wp_child_theme(){
		wp_enqueue_script('child-js', PC_PATH . 'js/custom-front.js', array( 'jquery' ), '1.0', true );
		wp_enqueue_style('bootstrapcss', PC_PATH . 'css/bootstrap.min.css');
		wp_enqueue_script('bootstrapminjs', PC_PATH . 'js/bootstrap.min.js');
		wp_enqueue_script('easingminjs', PC_PATH . 'js/jquery.easing.min.js');
global $woocommerce;
...
function wpdocs_selectively_enqueue_admin_script( $hook ) {
    wp_enqueue_script( 'my_custom_script', PC_PATH . 'js/repeater.js', array(), '1.0' );
	wp_enqueue_media();
	wp_enqueue_script( 'media_uploader', PC_PATH . 'js/media-uploader.js', array(), '1.0' );
    wp_enqueue_style( 'my_custom_style', PC_PATH . 'css/cst_style.css', array(), '1.0' );
...
if(!isset($_SESSION["tempcurtain_product_color_price"])){
	$_SESSION["tempcurtain_product_color_price"] = array();
}
if(!isset($_SESSION["curtain_product_color_price"])){
	$_SESSION["curtain_product_color_price"] = array();
}
if(!isset($_SESSION["closhwidth"])){
	$_SESSION["closhwidth"] = array();
}
if(!isset($_SESSION["closhwidth"])){
	$_SESSION["closhwidth"] = array();
}
if(!isset($_SESSION["fnlprice"])){
	$_SESSION["fnlprice"] = array();
}
if(!isset($_SESSION["temp_price"])){
	$_SESSION["temp_price"] = array();
}
if(!isset($_SESSION["curtain_product_color_name"])){
	$_SESSION["curtain_product_color_name"] = array();
}
if(!isset($_SESSION["product_fcolor_price"])){
	$_SESSION["product_fcolor_price"] = array();
}
	$_SESSION["tempcurtain_product_color_price"][$product_id] =  $_SESSION["curtain_product_color_price"][$product_id];
unset($_SESSION["curtain_product_color_price"][$product_id]);
	if(isset($_SESSION['closhwidth'][$product_id]))
   {
	 $closhwidth = $_SESSION["closhwidth"][$product_id];
   }
   if(isset($_SESSION['fnlprice'][$product_id]))
   {
	 $price = $_SESSION["fnlprice"][$product_id];
     $_SESSION["temp_price"][$product_id] = $_SESSION["fnlprice"][$product_id];
   }
    ...
    if($closhwidth<3500 && $colorkey=='cst_dynamic_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] = floatval($std_cst_curtaincolor_below);
   }
   if($closhwidth>=3500 && $colorkey=='cst_dynamic_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] = floatval($std_cst_curtaincolor_above);
   }
   if($closhwidth<3500 && $colorkey=='special_painted_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] = floatval($spc_cst_curtaincolor_below);
   }
   if($closhwidth>=3500 && $colorkey=='special_painted_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] = floatval($spc_cst_curtaincolor_above);
   }
   if($closhwidth<3500 && $colorkey=='laminate_wood_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] = floatval($lmt_cst_curtaincolor_below);
   }
   if($closhwidth>=3500 && $colorkey=='laminate_wood_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] =  floatval($lmt_cst_curtaincolor_above);
   }
   //white color
   if($closhwidth<3500 && $colorkey=='white_dynamic_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] = floatval($white_cst_curtaincolor_below);
   }
   if($closhwidth>=3500 && $colorkey=='white_dynamic_color_attr'){
	   $_SESSION["curtain_product_color_price"][$product_id] =  floatval($white_cst_curtaincolor_above);
   }
	   $_SESSION["curtain_product_color_name"][$product_id] = $color;
$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION["curtain_product_color_price"][$product_id]);
$sessionprice = abs(floatval($_SESSION["curtain_product_color_price"][$product_id])+ floatval($_SESSION["product_fcolor_price"][$product_id]));
	if(isset($_SESSION["tempcurtain_product_color_price"][$product_id]) && $_SESSION["tempcurtain_product_color_price"][$product_id] !==""){
		$_SESSION['fnlprice'][$product_id] = floatval($_SESSION['fnlprice'][$product_id]) - floatval($_SESSION["tempcurtain_product_color_price"]);
	}
	if(!isset($_SESSION['alrdadd'])){
		$_SESSION['alrdadd'] = array();
	}
	if(!isset($_SESSION['asscpprice'])){
		$_SESSION['asscpprice'] = array();
	}
	if(!isset($_SESSION['alrdaddcolor'])){
		$_SESSION['alrdaddcolor'] = array();
	}
	if(!isset($_SESSION['alrdadd'][$product_id])){
	$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION['asscpprice'][$product_id]);
	$_SESSION['alrdaddcolor'][$product_id] = $_SESSION['asscpprice'][$product_id];
}
$pricettl = $_SESSION['fnlprice'][$product_id];
...
function product_fcolorprice_ajax_request(){
	...
	if(!isset($_SESSION["tempproduct_fcolor_price"])){
		$_SESSION["tempproduct_fcolor_price"] = array();
	}
	if(!isset($_SESSION["product_fcolor_price"])){
		$_SESSION["product_fcolor_price"] = array();
	}
	...
	if(!isset($_SESSION["product_fcolor_name"])){
		$_SESSION["product_fcolor_name"] = array();
	}
$_SESSION["tempproduct_fcolor_price"][$product_id] = $_SESSION["product_fcolor_price"][$product_id];
unset($_SESSION["product_fcolor_price"][$product_id]);
	if(isset($_SESSION['closhwidth'][$product_id]))
   {
	 $closhwidth = $_SESSION["closhwidth"][$product_id];
   }
   ...
   $framestd_cst_curtaincolor_below = get_post_meta( $product_id, 'framestd_cst_curtaincolor_below', true );
    ...
    if($closhwidth<2800 && $colorkey=='framecst_dynamic_color_attr'){
	   $_SESSION["product_fcolor_price"][$product_id] = floatval($framestd_cst_curtaincolor_below);
   }
   if($closhwidth>=2800 && $colorkey=='framecst_dynamic_color_attr'){
	  $_SESSION["product_fcolor_price"][$product_id] = floatval($framestd_cst_curtaincolor_above);
   }
   if($closhwidth<2800 && $colorkey=='frame_special_painted_color_attr'){
	   $_SESSION["product_fcolor_price"][$product_id] = floatval($spc_fcst_framecolor_below);
   }
   if($closhwidth>=2800 && $colorkey=='frame_special_painted_color_attr'){
	  $_SESSION["product_fcolor_price"][$product_id] = floatval($spc_fcst_frame_above);
   }
   if($closhwidth<2800 && $colorkey=='frame_laminatewoodgrain_color_attr'){
	   $_SESSION["product_fcolor_price"][$product_id] = floatval($frame_laminatewoodgrain_below);
   }
   if($closhwidth>=2800 && $colorkey=='frame_laminatewoodgrain_color_attr'){
	   $_SESSION["product_fcolor_price"][$product_id] = floatval($frame_laminatewoodgrain_above);
   }
   //white color
   if($closhwidth<2800 && $colorkey=='white_dynamic_color_attr'){
	   $_SESSION["product_fcolor_price"][$product_id] = floatval($white_fcst_curtaincolor_below);
   }
   if($closhwidth>=2800 && $colorkey=='white_dynamic_color_attr'){
	   $_SESSION["product_fcolor_price"][$product_id] =  floatval($white_fcst_curtaincolor_above);
   }
	   $_SESSION["product_fcolor_name"][$product_id] = $color;
   $sessionprice = abs(floatval($_SESSION["curtain_product_color_price"][$product_id])+ floatval($_SESSION["product_fcolor_price"][$product_id]));
$_SESSION['fnlprice'][$product_id] = floatval($_SESSION['fnlprice'][$product_id]) + floatval($_SESSION["product_fcolor_price"][$product_id]);
if(isset($_SESSION["tempproduct_fcolor_price"][$product_id]) && $_SESSION["tempproduct_fcolor_price"][$product_id] !==""){
		$_SESSION['fnlprice'][$product_id] = floatval($_SESSION['fnlprice'][$product_id]) - floatval($_SESSION["tempproduct_fcolor_price"][$product_id]);
	}
if(!isset($_SESSION['alrdaddcolor'])){
	$_SESSION['alrdaddcolor'] = array();
}
...
if(!isset($_SESSION['alrdaddcolor'][$product_id])){
	$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION['asscpprice'][$product_id]);
}
$pricettl = $_SESSION['fnlprice'][$product_id];
	$gif_data = array(
...
function cst_save_val_session(){
	...
	if(!isset($_SESSION['other_opts'][$proid])){
	$_SESSION['other_opts'][$proid] = array();
	}
	foreach($_POST as $key => $postvar){
		if($key == "product_id"){
			continue;
		}
		$_SESSION['other_opts'][$proid][$key] = $postvar;
	}
}
...
function product_changeprice_ajax_request(){
	...
	if(!isset($_SESSION['fortempfnl'])){
		$_SESSION['fortempfnl'] = array();
	}
	if(!isset($_SESSION['cur_cust_prod_id'])){
		$_SESSION['cur_cust_prod_id'] = array();
	}
	if(isset($_SESSION['fnlprice'][$product_id])){
$_SESSION['fortempfnl'][$product_id] = $_SESSION['fnlprice'][$product_id];
	}
	else{
	$_SESSION['fortempfnl'][$product_id] = 0;
	}
...
foreach($value as $vkey => $vvalue){
	...
	print_r($value);
	echo "</pre>"; */
	unset($_SESSION[$key][$vkey]);
	}
}
}
unset($_SESSION['accessories_for_'.$product_id]);
if(!isset($_SESSION['cur_cust_prod_id'][$product_id])){
	$_SESSION['cur_cust_prod_id'][$product_id] = $_POST['product_id'];
}
...
if(!isset($_SESSION["product_installation_price"])){
	$_SESSION["product_installation_price"] = array();
}
if(!isset($_SESSION["product_fcolor_price"])){
	$_SESSION["product_fcolor_price"] = array();
}
...
if(!isset($_SESSION["alrdproduct_fcolor_price"])){
	$_SESSION["alrdproduct_fcolor_price"] = array();
}
if(!isset($_SESSION["asscpprice"])){
	$_SESSION["asscpprice"] = array();
}
...
if(!isset($_SESSION["closheight"])){
	$_SESSION["closheight"] = array();
}
if(!isset($_SESSION["tempwidth"])){
	$_SESSION["tempwidth"] = array();
}
if(!isset($_SESSION["tempheight"])){
	$_SESSION["tempheight"] = array();
}
	$_SESSION["fnlprice"][$product_id] = $findoorprice;
	$sendarr = array('main'=> $findoorprice, 'ttl' => $findoorprice);
if(isset($_SESSION["curtain_product_color_price"][$product_id])){
	$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION["curtain_product_color_price"][$product_id]);
	$sendarr['ttl'] = $_SESSION["fnlprice"][$product_id];
}
if(isset($_SESSION["product_installation_price"][$product_id])){
	$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION["product_installation_price"][$product_id]);
	$sendarr['ttl'] = $_SESSION["fnlprice"][$product_id];
}
if(isset($_SESSION["product_fcolor_price"][$product_id])){
	$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION["product_fcolor_price"][$product_id]);
	$sendarr['ttl'][$product_id] = $_SESSION["fnlprice"][$product_id];
	$_SESSION["alrdproduct_fcolor_price"][$product_id] = $_SESSION["product_fcolor_price"][$product_id];
}
if(isset($_SESSION['asscpprice'][$product_id])){
	$_SESSION["fnlprice"][$product_id] = floatval($_SESSION["fnlprice"][$product_id]) + floatval($_SESSION['asscpprice'][$product_id]);
	$sendarr['ttl'][$product_id] = $_SESSION["fnlprice"][$product_id];
	$_SESSION["alrdadd"][$product_id] = $_SESSION['asscpprice'][$product_id];
}
echo json_encode($sendarr);
	$_SESSION["closhwidth"][$product_id] = $closhwidth;
	$_SESSION["closheight"][$product_id] = $closheight;
	$_SESSION["tempwidth"][$product_id] = $width;
	$_SESSION["tempheight"][$product_id] = $height;
	exit;
}
add_filter( 'woocommerce_get_cart_item_from_session', function ( $cartItemData, $cartItemSessionData, $cartItemKey ) {
    if ( isset( $cartItemSessionData['cst_custom_data'] ) ) {
        $cartItemData['cst_custom_data'] = $cartItemSessionData['cst_custom_data'];
    }
	$pro_id = $cartItemData['product_id'];
	if($_SESSION['cur_cust_prod_id'][$pro_id]){
$cartItemData['data']->set_price( $cartItemData['cst_custom_data']['fnlpriceprows'] );
	}
    return $cartItemData;
}, 10, 3 );
...
if(!isset($_SESSION['procount'])){
	$_SESSION['procount'] = array();
}
if(!isset($_SESSION['upfnlprice'])){
	$_SESSION['upfnlprice'] = array();
}
if(isset($_SESSION['cur_cust_prod_id'][$product->get_id()])){
					$new_price = $price;
}
else{
	$new_price = $price;
...
function add_custom_fields_cart_item_data( $cart_item_data, $product_id ){
   if (!session_id()){
	...
	$cart_item_data['cst_custom_data']['opening_width'] = $_SESSION["tempwidth"][$product_id]." mm";
	$cart_item_data['cst_custom_data']['opening_height'] = $_SESSION["tempheight"][$product_id]." mm";
		if(isset($_SESSION['other_opts'][$product_id])){
			foreach($_SESSION['other_opts'][$product_id] as $key=>$val){
				if($key == 'action'|| $key == 'mtrnxt'){
					continue;
				}
				$_SESSION['save_in_ordermeta'][$product_id][$key] = $val;
					$cart_item_data['cst_custom_data'][str_replace(" ", "_", strtolower($key))] = $val;
			}
		}
	if(isset($_SESSION['curtain_product_color_name'][$product_id])){
	$cart_item_data['cst_custom_data']['curtain'] = $_SESSION['curtain_product_color_name'][$product_id];
	}
	if(isset($_SESSION['product_fcolor_name'][$product_id])){
	$cart_item_data['cst_custom_data']['frame'] = $_SESSION['product_fcolor_name'][$product_id];
	}
	if(isset($_SESSION['product_installation_price'][$product_id])){
	$cart_item_data['cst_custom_data']['installation_price'] = $_SESSION['product_installation_price'][$product_id];
	}
	if(isset($_SESSION['upfnlprice'][$product_id])){
	$cart_item_data['cst_custom_data']['fnlpriceprows'] = $_SESSION['upfnlprice'][$product_id];
	}
WC()->session->set( 'cst_custom_data_sess', $cart_item_data['cst_custom_data'] );
...
$temp_var = $_SESSION['temp_wc_sess_'.$cart_item_key];
    // Save values:
foreach($temp_var as $key=>$val){
	$item->update_meta_data( __(ucfirst(str_replace("_", " ", $key)), $text_domain), $val );
}
}
update_option("temp_wc_sess_".$cart_item_key, $_SESSION['temp_wc_sess_'.$cart_item_key]);
}
...
echo "</pre>"; */
	...
	if(isset($_SESSION['cur_cust_prod_id'][$proid])){
		$sessionpro = intval($_SESSION['cur_cust_prod_id'][$proid]);
		if($cart_item['product_id'] == $sessionpro){
	$_SESSION['save_in_ordermeta'][$proid]=array();
	if(isset($_SESSION['curtain_product_color_price'][$proid]))
   {
	 $pcolor_price = $_SESSION["curtain_product_color_price"][$proid];
	 $pcolor_name = $_SESSION["curtain_product_color_name"][$proid];
   }
	$custom_items[] = array(
            'name'      => 'Opening Width',
            'value'     => $cart_item['cst_custom_data']['opening_width'],
        );
		$_SESSION['save_in_ordermeta'][$proid]['opening_width'] = $cart_item['cst_custom_data']["opening_width"]." mm";
	$custom_items[] = array(
            'name'      => 'Opening Height',
            'value'     => 	$cart_item['cst_custom_data']['opening_height'],
        );
		$_SESSION['save_in_ordermeta'][$proid]['opening_height'] = $cart_item['cst_custom_data']['opening_height']." mm";
		//var_dump($_SESSION['other_opts']);
		if(isset($_SESSION['other_opts'][$proid])){
			foreach($_SESSION['other_opts'][$proid] as $key=>$val){
				if($key == 'action'|| $key == 'mtrnxt'){
					continue;
				}
				$_SESSION['save_in_ordermeta'][$proid][$key] = $val;
					$custom_items[] = array(
					'name'      => ucFirst($key),
					'value'     => $cart_item['cst_custom_data'][$key],
					);
			}
		}
	if(isset($_SESSION['curtain_product_color_name'][$proid])){
	$custom_items[] = array(
            'name'      => 'Curtain',
            'value'     => $cart_item['cst_custom_data']['curtain'],
        );
			$_SESSION['save_in_ordermeta'][$proid]['curtain'] = $cart_item['cst_custom_data']['curtain'];
	}
	if(isset($_SESSION['product_fcolor_name'][$proid])){
	$custom_items[] = array(
            'name'      => 'Frame',
            'value'     => $cart_item['cst_custom_data']['frame'],
        );
		$_SESSION['save_in_ordermeta'][$proid]['frame'] = $cart_item['cst_custom_data']['frame'];
	}
	$custom_items[] = array(
            'name'      => 'Installation Price',
            'value'     => $cart_item['cst_custom_data']['installation_price'],
        );
	$custom_items[] = array(
            'name'      => 'Final Price',
            'value'     => $cart_item['cst_custom_data']['fnlpriceprows'],
        );
		$_SESSION['save_in_ordermeta'][$proid]['product_installation_price'] = $cart_item['cst_custom_data']["product_installation_price"];
	$itemky = $cart_item['key'];
	$_SESSION['product_installation_price_with_key'][$itemky] = $cart_item['cst_custom_data']['installation_price'];
	$_SESSION['temp_wc_sess_'.$itemky] = $cart_item['cst_custom_data'];
	/* rerandomize :
	$randnum = rand(0,100000);
	if(!isset($_SESSION['randnums'])){
		$_SESSION['randnums'] = array();
	}
	if(!in_array($randnum, $_SESSION['randnums'])){
	$_SESSION['randnums'][] = $randnum;
	$_SESSION['cst_cart_item_price'][$randnum] = $_SESSION['upfnlprice'][$proid];
	}
	else{
		goto rerandomize;
	} */
      ...
      <div class="panel-body">
		 ...
		 <div class="col-md-6">
		   <label  class="cst_labels">Opening Width (mm)</label>
			...
			$cst_dynamic_pricing = get_post_meta( $post->ID, 'cst_dynamic_pricing', true );
			foreach($cst_dynamic_pricing as $cst_dynamic_price){
				echo $cst_dynamic_price[0]['doorheight'];
			}
			
			echo "<pre>"; print_r($cst_dynamic_pricing); 
			?>
  ...
  <div class="panel panel-default">
    ...
    <div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
      <div class="panel-body">
		...
		<a data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne" class="btn btn-default prev">Previous</a>
  ...
  <div class="panel panel-default">
    ...
    <div id="collapseThree" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
      <div class="panel-body">
		...
		?>
			 <a data-toggle="collapse" data-parent="#accordion" href="#<?php echo $collapsep3; ?>" aria-expanded="true" aria-controls="<?php echo $collapsep3; ?>" class="btn btn-default prev">Previous</a>
  ...
  <div class="panel panel-default">
    ...
    <div id="collapseFour" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
      <div class="panel-body">
		...
		?>
			  <a data-toggle="collapse" data-parent="#accordion" href="#<?php echo $collapsep4; ?>" aria-expanded="true" aria-controls="<?php echo $collapsep4; ?>" class="btn btn-default prev">Previous</a>
 ...
 <div class="panel panel-default">
    ...
    <div id="collapseFive" class="panel-collapse collapse " role="tabpanel" aria-labelledby="headingOne">
      <div class="panel-body">
		...
		?>
			 <a data-toggle="collapse" data-parent="#accordion" href="#<?php echo $collapsep5; ?>" aria-expanded="true" aria-controls="<?php echo $collapsep5; ?>" class="btn btn-default prev">Previous</a>
...
<?php  if($white_dynamic_color_attr_enabled || $cst_dynamic_color_attr_enabled || $special_painted_color_attr_enabled || $laminate_wood_color_attr_enabled){ ?>
  <div class="panel panel-default colour">
    <div class="panel-heading" role="tab" id="headingOne">
      <h4 class="panel-title">
        <a class="collapsed">
 ...
 if($cst_dynamic_color_attr_enabled){
		...
		?>
        <a data-toggle="collapse" data-parent="#accordion" href="#<?php echo $collapsep6; ?>" aria-expanded="true" aria-controls="<?php echo $collapsep6; ?>" class="btn btn-default prev">Previous</a>
...
<?php if($white_dynamic_color_attr_enabled || $framecst_dynamic_color_attr_enabled || $frame_special_painted_color_attr_enabled || $frame_laminatewoodgrain_color_attr_enabled){ ?>
  <div class="panel panel-default colour">
	...
	if($white_dynamic_color_attr_enabled || $cst_dynamic_color_attr_enabled || $special_painted_color_attr_enabled || $laminate_wood_color_attr_enabled){
		...
		?>
        <a data-toggle="collapse" data-parent="#accordion" href="#<?php echo $collapsep7; ?>" aria-expanded="true" aria-controls="<?php echo $collapsep7; ?>" class="btn btn-default prev">Previous</a>
...
<div class="panel panel-default">
    ...
    <div id="collapseNine" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
      <div class="panel-body">
	     ...
	     <a data-toggle="collapse" data-parent="#accordion" href="#collapseEight" aria-expanded="true" aria-controls="collapseEight" class="btn btn-default prev">Previous</a>
			 <button type="submit" class="button cst_addtocart" cst-addid="<?php echo get_the_ID(); ?>" disabled>Add To Basket</button>
  </div>
</div>
</div>
 <?php } ?>
...
function cst_unset_session(){
	...
	$_SESSION['upfnlprice'] = $_SESSION['fnlprice'];
/* 	echo "<pre>";
	print_r($_SESSION);
	echo "</pre>"; */
}
...
function cst_unset_session_on_head(){
	...
	/* foreach($_SESSION['fnlprice'] as $key=>$fnlprc){
		$_SESSION['fnlprice'][$key] = 0;
	} */
	}
}
...
function wp_ajax_product_accss_ajax_request(){
	...
	global $woocommerce;
	$proid = $_POST['pro_id'];
	$poidarry = array(0=>"none");
	if(isset($_POST['idarr'])){
		$iddrr = array_merge($poidarry, $_POST['idarr']);
	}
	else{
		$iddrr = $poidarry ;
	}
	if(!isset($_SESSION['accessories_for_'.$proid])){
	$_SESSION['accessories_for_'.$proid] = array(0=>"none");
	}
	$fnlprc = $_POST['fnlprc'];
	//$woocommerce->cart->empty_cart();
	foreach($_POST['idarr'] as $pstid){
		if(!in_array($pstid, $_SESSION['accessories_for_'.$proid])){
			$_SESSION['accessories_for_'.$proid][] = $pstid;
		$woocommerce->cart->add_to_cart( intval($pstid), 1 );
		}
	}
	$removed_accessories = array_diff($_SESSION['accessories_for_'.$proid],$iddrr);
	if(!empty($removed_accessories)){
		foreach($removed_accessories as $removed_accessory){
			$in_cart = false;
			foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				if ( $cart_item['product_id'] === intval($removed_accessory) ) {
					$in_cart = true;
					$key = $cart_item_key;
					WC()->cart->remove_cart_item( $key );
					$_SESSION['accessories_for_'.$proid] = array_diff($_SESSION['accessories_for_'.$proid], array($removed_accessory));
				}
			}
		}
	}
	$pricettl = floatval($_SESSION['fnlprice'][$proid]) + floatval($fnlprc);
	//$_SESSION['fnlprice'] = $pricettl;
	if(!isset($_SESSION['asscpprice'])){
		$_SESSION['asscpprice'] = array();
	}
	$_SESSION['asscpprice'][$proid] = $fnlprc;
	echo $pricettl ;
	exit();
}
add_action('woocommerce_thankyou', 'cst_unset_sess');
function cst_unset_sess($order_id){
...
function cst_save_ordermeta( $order_id, $posted ) {
	...
	$order_meta = array();
	foreach($_SESSION['save_in_ordermeta'] as $prokey => $ordrmta){
	foreach($ordrmta as $key => $ordrmtaval){
...
function product_instaopt_ajax_request(){
	...
	if(isset($_SESSION["product_installation_price"][$post_id])){
	$fnlprice = floatval($_SESSION["fnlprice"][$post_id]);
	$fnlprice = $fnlprice-$_SESSION["product_installation_price"][$post_id];
	$_SESSION["fnlprice"][$post_id] = $fnlprice;
    }
	$sentarr['ttlval'] = floatval($_SESSION['fnlprice'][$post_id]) + floatval($val) + floatval($_SESSION['asscpprice'][$post_id]);
	$_SESSION['fnlprice'][$post_id] = floatval($_SESSION['fnlprice'][$post_id]) + floatval($val);
	$sentarr['val'] = floatval($val);
	$_SESSION['product_installation_price'][$post_id] = floatval($val);
	/* rerandomize :
	$randnum = rand(0,100000);
	if(!isset($_SESSION['randnums'])){
		$_SESSION['randnums'] = array();
...
global $woocommerce;
$freeship = false;
foreach($_SESSION['product_installation_price_with_key'] as $pkey => $pipwithkey){
	$items = $woocommerce->cart->get_cart();
	if(!isset($items[$pkey])){
		unset($_SESSION['product_installation_price_with_key'][$pkey]);
		continue;
	}
	if($pipwithkey !=0 && $pipwithkey!="" ){
		$freeship = true;
		break;
	}
}
...
function my_field_order_meta_handler( $item_id, $values, $cart_item_key ) {
   	$product_id = $item_id;
	if( ! isset($values['cst_custom_data_sess']) )
        return;
    $text_domain ='woocommerce';
    // Save values:
	wc_add_order_item_meta( $item_id, "cst_custom_data_sess", $values['cst_custom_data_sess'] );
}
